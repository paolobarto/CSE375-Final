...
//TODO refactor to my own design
  ...
  function<bool(int, atomic<int>*)> storedHandler;
  ...
  void setup(){
    ...
    while(!shutdown.load()){
      ...
      if (sd != -1 && !shutdown.load()) {
        tempsh = this->storedHandler(sd, trainingTreads); //runs handler on sd
      }
  ...
  my_pool(int size, function<bool(int, atomic<int>*)> handler) {
...
thread_pool *pool_factory(int size, function<bool(int, atomic<int>*)> func) {
  return new my_pool(size, func);
}
