...
/// must be added to the file, describing the change.  There are six commands
/// that can cause a change: REG, SET, CPW, KVI, KVU, and KVD.  Note that REG can
...
/// only require new entries for CPW, SET, KVD, and a KVU that results in an update.
/// The formats that correspond to these entries are defined below:
///
/// New password entry format:
/// - 8-byte constant AUTHCHPW
/// - 8-byte binary write of the length of the username
/// - Binary write of the bytes of the username
/// - 8-byte binary write of the length of the new salt
/// - Binary write of the bytes of the new salt
/// - 8-byte binary write of the length of the new hashed password
/// - Binary write of the bytes of the new hashed password
/// - Binary write of some bytes of padding, to ensure that the next entry will
///   be aligned on an 8-byte boundary.
///
...
/// table entry passwrod
const std::string AUTHCHPW = "AUTHCHPW";
